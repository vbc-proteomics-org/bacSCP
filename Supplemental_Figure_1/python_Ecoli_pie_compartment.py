...
mask_c = df[col_goslim].astype(str).str.replace('_','.', regex=False).str.upper().str.contains('GOSLIM.C')
dfc = df[mask_c].copy()

agg_all = dfc.groupby([col_group, col_term], dropna=False)[col_count].sum().reset_index()
all_terms = sorted(agg_all[col_term].astype(str).unique())

cmap = plt.get_cmap(palette, len(all_terms))
...
for label, keywords in supergroups.items():
    mask = dfc[col_group].astype(str).str.lower().apply(lambda g: any(kw.lower() in g for kw in keywords))
    subset = dfc[mask]
...
fig.suptitle('Cellular Component', fontsize=15, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig("compartment.png", bbox_inches='tight')
